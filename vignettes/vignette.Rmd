...
Subsetting a `BSFinderData` can be useful in a variatey of cases, eg. reducing the object size for faster parameter testing, limiting the analysis to some candidate genes ect.. Here we subset the object by a random set of 100 binding sites and plot their count distribution.
...
bindingSiteDetailsPlot(bdsRemovedRanges, plotIdx = 2, flankPos = 50)
bindingSiteDetailsPlot(bdsRemovedRanges, plotIdx = 2, flankPos = 50, mergeReplicates = TRUE)
```
...
bindingSiteDetailsPlot(bdsFinal, plotIdx = idxMaxCountsBs, flankPos = 100, mergeReplicates = FALSE, shiftPos = 50)
```
